...
fn alloc(ref allocator: AllocatorState, size_bytes: u128, count: u128) -> Address {
    let size_bytes: ByteCount = size_bytes.try_into().expect('size_bytes does not fit u64');
    let count: ByteCount = count.try_into().expect('count does not fit u64');
...
pub fn __llvm_alloc_l_l_p(ref state: RTState, size_bytes: u128, count: u128) -> Address {
