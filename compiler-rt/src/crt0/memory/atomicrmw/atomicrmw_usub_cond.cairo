...
use core::num::traits::{BitSize, Bounded, OverflowingMul, WrappingSub};
...
/// subtract the given value from the value in memory if the value in memory is lower or equal than
/// the given value. Store the subtraction result. Unsigned comparison is performed. The value
/// loaded from the memory is returned. The number of loaded and stored bytes is equal to the size
/// of T.
///
...
///
/// This is the implementation of the `__llvm_atomicrmw_usub_cond_*` polyfills logic. It exists so
...
mod test {
    ...
    ///
    /// Perform the `atomicrmw usub_cond` operation on the first sizeof(T) bytes of the allocated
    /// memory region, using the given value.
    /// Check if the value stored in the memory at the end of the operation has changed if it was
    /// necessary.
    fn test_atomicrmw_usub_cond<
        T,
        +Bounded<T>,
        +BitAnd<T>,
        +BitOr<T>,
    ...
    >(
        ...
        // Do the operation.
        let old_value = atomicrmw_usub_cond::<T>(ref allocator, address, value);
    ...
    #[test]
    /// Test the `atomicrmw usub_cond` operation with u8 values.
    fn atomicrmw_usub_cond_u8() {
        test_atomicrmw_usub_cond::<u8>(0x00);
        test_atomicrmw_usub_cond::<u8>(0xff);
    }

    #[test]
    /// Test the `atomicrmw usub_cond` operation with u16 values.
    fn atomicrmw_usub_cond_u16() {
        test_atomicrmw_usub_cond::<u16>(0x0000);
        test_atomicrmw_usub_cond::<u16>(0xffff);
    }
    ...
    #[test]
    /// Test the `atomicrmw usub_cond` operation with u32 values.
    fn atomicrmw_usub_cond_u32() {
        test_atomicrmw_usub_cond::<u32>(0x00000000);
        test_atomicrmw_usub_cond::<u32>(0xffffffff);
    }
    ...
    #[test]
    /// Test the `atomicrmw usub_cond` operation with u64 values.
    fn atomicrmw_usub_cond_u64() {
        test_atomicrmw_usub_cond::<u64>(0x0000000000000000);
        test_atomicrmw_usub_cond::<u64>(0xffffffffffffffff);
    }

    #[test]
    /// Test the `atomicrmw usub_cond` operation with u128 values.
    fn atomicrmw_usub_cond_u128() {
        test_atomicrmw_usub_cond::<u128>(0x00000000000000000000000000000000);
        test_atomicrmw_usub_cond::<u128>(0xffffffffffffffffffffffffffffffff);
    }
}

pub fn __llvm_atomicrmw_usub_cond_p_b_b(address: Address, value: u8) -> u8 {
    let mut allocator = get_allocator().unbox();
    atomicrmw_usub_cond(ref allocator, address, value)
}

pub fn __llvm_atomicrmw_usub_cond_p_z_z(address: Address, value: u16) -> u16 {
    let mut allocator = get_allocator().unbox();
    atomicrmw_usub_cond(ref allocator, address, value)
}

pub fn __llvm_atomicrmw_usub_cond_p_x_x(address: Address, value: u24) -> u24 {
    let mut allocator = get_allocator().unbox();
    atomicrmw_usub_cond(ref allocator, address, value)
}

pub fn __llvm_atomicrmw_usub_cond_p_i_i(address: Address, value: u32) -> u32 {
    let mut allocator = get_allocator().unbox();
    atomicrmw_usub_cond(ref allocator, address, value)
}

pub fn __llvm_atomicrmw_usub_cond_p_n_n(address: Address, value: u40) -> u40 {
    let mut allocator = get_allocator().unbox();
    atomicrmw_usub_cond(ref allocator, address, value)
}

pub fn __llvm_atomicrmw_usub_cond_p_k_k(address: Address, value: u48) -> u48 {
    let mut allocator = get_allocator().unbox();
    atomicrmw_usub_cond(ref allocator, address, value)
}

pub fn __llvm_atomicrmw_usub_cond_p_l_l(address: Address, value: u64) -> u64 {
    let mut allocator = get_allocator().unbox();
    atomicrmw_usub_cond(ref allocator, address, value)
}

pub fn __llvm_atomicrmw_usub_cond_p_o_o(address: Address, value: u128) -> u128 {
    let mut allocator = get_allocator().unbox();
    atomicrmw_usub_cond(ref allocator, address, value)
}
