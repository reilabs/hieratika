...
pub fn __llvm_icmp_slt_c_c_c(ref state: RTState, lhs: u128, rhs: u128) -> u128 {
    ...
    // This is a signed comparison and we're working in u8 domain, so sign extend.
    #[cairofmt::skip]
    let lhs = if lhs == 1 { 0b11111111 } else { 0 };
    #[cairofmt::skip]
    let rhs = if rhs == 1 { 0b11111111 } else { 0 };
...
mod tests {
    ...
    use super::__llvm_icmp_slt_c_c_c;
    ...
    fn test_i1() {
        for case in test_cases.span() {
            ...
            assert_eq!(__llvm_icmp_slt_c_c_c(ref state, *case.lhs, *case.rhs), *case.expected);
        }
    }
}
