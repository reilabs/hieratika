...
pub fn emit_sierra_for_flo(
    ...
    db.set_crate_config(
        ...
        Some(CrateConfiguration {
            root:       Directory::Real(infer_compiler_rt_path().unwrap()),
    ...
    let mut current_main_crate_config = db.crate_config(main_crate_id).unwrap();
    ...
    let sierra_program =
        compile_prepared_db_program(&mut db, vec![main_crate_id], compiler_config).unwrap();
